...
{
    RCLibDbPlaylistIter *reference, *iter;
    ...
    if(!rclib_db_playlist_is_valid_iter(reference)) return;
    iter = reference;
    rclib_db_playlist_data_iter_get(iter, RCLIB_DB_PLAYLIST_DATA_TYPE_CATALOG,
        catalog_iter, RCLIB_DB_PLAYLIST_DATA_TYPE_NONE);
    if(catalog_iter==NULL) return;
    if(priv->limit_state)
    {
        iter_array = g_ptr_array_new();
        for(foreach_iter=rclib_db_playlist_get_begin_iter(catalog_iter);
    ...
    {
        length = rclib_db_playlist_iter_get_length(iter);
        if(length<=0)
        {
            rclib_player_play_db(iter);
            return;
        ...
        iter = rclib_db_playlist_iter_get_iter_at_pos(reference, pos);
        if(iter==NULL) return;
        rclib_player_play_db(iter);
    }
}
...
static inline void rclib_player_random_all_play(RCLibPlayerPrivate *priv)
{
        ...
        catalog_iter = rclib_db_catalog_iter_next(catalog_iter))
    {
        if(priv->limit_state)
        {
            ...
            total_length += rclib_db_playlist_get_length(catalog_iter);
    }
...
static inline void rclib_player_repeat_list(RCLibPlayerPrivate *priv)
{
    ...
    if(priv->limit_state)
    {
        ...
        for(foreach_iter=rclib_db_playlist_iter_get_begin_iter(reference);
    ...
    else
    {
        if(rclib_db_playlist_iter_is_end(iter))
            iter = rclib_db_playlist_iter_get_begin_iter(reference);
        rclib_player_play_db(iter);
    }
}
    ...
    if(priv->limit_state)
    {
            ...
            cforeach_iter=rclib_db_catalog_iter_next(cforeach_iter))
        {
            for(iter = rclib_db_playlist_get_begin_iter(cforeach_iter);
                !rclib_db_playlist_iter_is_end(iter);
                iter=rclib_db_playlist_iter_next(iter))
            {
                rating = -1.0;
    ...
    gboolean loop)
{
    ...
    if(rclib_db_playlist_iter_is_begin(iter))
    {
        ...
        if(rclib_db_catalog_iter_is_begin(catalog_iter))
        {
            ...
            catalog_iter = rclib_db_catalog_get_end_iter();
            do
            {
            ...
            iter = rclib_db_playlist_iter_prev(
                rclib_db_playlist_get_end_iter(catalog_iter));
            if(iter!=NULL)
                rclib_player_play_db(iter);
            return TRUE;
        }
