...
static GstPadProbeReturn rclib_core_effect_add_element_block_probe_cb(
    GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
    GstElement *effectbin, *bin;
    GstPad *realpad, *prevpad;
    GstPad *binsrcpad, *binsinkpad;
    GstPadLinkReturn link;
    GstElement *identity;
    gboolean linking;
    if(user_data==NULL)
    {
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    linking = GPOINTER_TO_UINT(((gpointer *)user_data)[2]);
    if(linking)
    {
        return GST_PAD_PROBE_OK;
    }
    effectbin = GST_ELEMENT(((gpointer *)user_data)[0]);
    bin = (GstElement *)((gpointer *)user_data)[1];
    ((gpointer *)user_data)[2] = GUINT_TO_POINTER(1);
    if(!GST_IS_BIN(bin))
    {
        g_warning("Invalid child bin for effect element!");
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    if(!gst_object_has_ancestor(GST_OBJECT(bin), GST_OBJECT(effectbin)))
    {
        g_warning("Not added into the effectbin!");
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    binsinkpad = gst_element_get_static_pad(bin, "sink");
    binsrcpad = gst_element_get_static_pad(bin, "src");
    if(binsinkpad==NULL || binsrcpad==NULL)
    {
        if(binsinkpad!=NULL) gst_object_unref(binsinkpad);
        if(binsrcpad!=NULL) gst_object_unref(binsrcpad);
        g_warning("Cannot add new element by obtaining pads from new bin!");
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    ...
    if(identity==NULL)
    {
        ...
        gst_object_unref(binsrcpad);
        g_warning("Cannot add new element, missing effect-identity element!");
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    ...
    if(link!=GST_PAD_LINK_OK)
    {
        ...
        gst_object_unref(binsinkpad);
        gst_object_unref(binsrcpad);
        g_warning("Cannot add new element, rollbacked linking.");
    ...
    if(link!=GST_PAD_LINK_OK)
    {
        gst_pad_unlink(prevpad, binsinkpad);
        gst_pad_link(prevpad, realpad);
        gst_object_unref(prevpad);
        gst_object_unref(realpad);
        gst_object_unref(binsinkpad);
    ...
    if(info!=NULL)
    {
        gst_element_set_state(effectbin, GST_STATE_PLAYING);
        gst_pad_remove_probe(pad, info->id);
    }
    else
    {
    ...
    g_message("Added new element to effectbin.");
    return GST_PAD_PROBE_OK;
}

static gboolean rclib_core_effect_add_element_internal(GstElement *effectbin,
    GstElement *new_element, const gchar *bin_name, gboolean block)
{
    GstPad *binsinkpad;
    GstPad *binsrcpad;
    GstPad *realpad;
    GstPad *effect_sinkpad = NULL;
    GstElement *bin = NULL;
    ...
    if(bin_name!=NULL)
    {
        bin = gst_bin_get_by_name(GST_BIN(effectbin), bin_name);
        if(bin!=NULL)
        {
            g_warning("The same element has been added!");
    ...
    G_STMT_START
    {
        ...
        gst_object_unref(realpad);
        effect_sinkpad = gst_element_get_static_pad(effectbin, "sink");
        if(effect_sinkpad==NULL)
        {
            break;
        }
        flag = TRUE;
    ...
    user_data[0] = effectbin;
    user_data[1] = bin;
    user_data[2] = NULL;
    if(block)
    {
        probe_id = gst_pad_add_probe(effect_sinkpad,
            GST_PAD_PROBE_TYPE_BLOCK_DOWNSTREAM,
            rclib_core_effect_add_element_block_probe_cb, user_data, g_free);
    }
    else
    {
        rclib_core_effect_add_element_block_probe_cb(effect_sinkpad,
            NULL, user_data);
        g_free(user_data);
        probe_id = 1;
    }
    gst_object_unref(effect_sinkpad);
    return (probe_id>0);
}

static GstPadProbeReturn rclib_core_effect_remove_element_block_probe_cb(
    GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
    GstElement *effectbin, *element, *bin;
    GstPad *nextpad, *prevpad, *mypad;
    gboolean linking;
    if(user_data==NULL)
    {
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
    ...
    effectbin = GST_ELEMENT(((gpointer *)user_data)[0]);
    element = (GstElement *)((gpointer *)user_data)[1];
    ((gpointer *)user_data)[2] = GUINT_TO_POINTER(1);
    
    if(!GST_IS_ELEMENT(element))
    {
        g_warning("Invalid element to remove!");
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    ...
    if(bin==NULL)
    {
        g_warning("Cannot find the parent bin for the element to remove!");
        if(info!=NULL) gst_pad_remove_probe(pad, info->id);
        return GST_PAD_PROBE_OK;
    }
    ...
    if(mypad!=NULL && prevpad!=NULL && GST_IS_PAD(mypad) &&
        GST_IS_PAD(prevpad))
    {
    ...
    if(mypad!=NULL && nextpad!=NULL && GST_IS_PAD(mypad) &&
        GST_IS_PAD(nextpad))
    {
    ...
    if(prevpad!=NULL && nextpad!=NULL && GST_IS_PAD(prevpad) &&
        GST_IS_PAD(nextpad))
    {
    ...
    g_message("Removed a element from effectbin.");
    return GST_PAD_PROBE_OK;
}
...
{
    GstPad *effect_sinkpad;
    gpointer *user_data;
    effect_sinkpad = gst_element_get_static_pad(effectbin, "sink");
    if(effect_sinkpad==NULL)
    {
        g_warning("Cannot obtain the sink pad of effectbin!");
        return;
    }
    ...
    if(block)
    {
        gst_pad_add_probe(effect_sinkpad, GST_PAD_PROBE_TYPE_BLOCK_DOWNSTREAM,
            rclib_core_effect_remove_element_block_probe_cb, user_data, g_free);
    }
    else
    {
        rclib_core_effect_remove_element_block_probe_cb(effect_sinkpad,
            NULL, user_data);
        g_free(user_data);
    }
    gst_object_unref(effect_sinkpad);
...
static void rclib_core_instance_init(RCLibCore *core)
{
    ...
    if(priv->bal_plugin!=NULL)
    {
        ...
        rclib_core_effect_add_element_internal(effectbin, priv->bal_plugin,
            "rc2-effectbin-audiopanorama", FALSE);
    }
